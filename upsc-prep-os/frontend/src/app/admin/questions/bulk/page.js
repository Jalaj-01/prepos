...
// Component logic is defined here
function BulkImporterContent() {
    const [rawText, setRawText] = useState('');
    const [parsedQuestions, setParsedQuestions] = useState([]);
    const [loading, setLoading] = useState(false);
    ...
    const handleFileUpload = async (e) => {
        ...
        setExtracting(true);
        try {
            // Dynamically import the helper only when the user actually uploads a file
            const { extractTextFromPDF } = await import('@/lib/pdfHelper');
            const text = await extractTextFromPDF(file);
            setRawText(text);
        } catch (err) {
...
// 3. Final Export wrapping the component to prevent Build Errors
export default function BulkImporter() {
