    ...
    const googleSuccess = async (response) => {
        ...
        try {
            const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/google`, { 
                ...
                targetCompletionDate: formData.targetCompletionDate // Passing the date here
            });
            localStorage.setItem('userInfo', JSON.stringify(data));
            router.push('/dashboard');
        } catch (err) {
        ...
        <div className="min-h-screen bg-brand-light flex items-center justify-center p-4">
            ...
            >
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-black text-brand-dark tracking-tight">Join PrepOS</h1>
                    <p className="text-brand-muted text-sm font-medium mt-2">Initialize your systematic preparation</p>
                </div>
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <span className="text-[10px] font-black text-brand-muted uppercase tracking-widest">or use email</span>
                    <div className="h-[1px] bg-brand-border flex-1"></div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    >
                        {loading ? "Creating Profile..." : "Initialize Preparation"} <ArrowRight size={20}/>
                    </button>
                </form>

                <p className="text-center text-sm text-brand-muted mt-8 font-medium">
                    Already a member? <Link href="/login" className="text-brand-accent font-black hover:underline">Sign In</Link>
                </p>
            </motion.div>
        </div>
    );
